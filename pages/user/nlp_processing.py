...
# def tokenize_input(keyword, stop_words, important_keywords):
#     try:
#         # Konversi keyword ke lowercase
#         lower_keyword = keyword.lower()
...
#     # Pastikan argumen keyword disediakan
#     if len(sys.argv) != 2:
#         print("Usage: python nlp_processing.py <json_data>")
#         sys.exit(1)

#     # Baca data JSON dari argumen pertama script
#     data = json.loads(sys.argv[1])
#     keyword = data['keyword']
#     stop_words = set(data['stop_words'])
#     important_keywords = set(data['important_keywords'])

#     # Jalankan fungsi tokenize_input
#     processed_keyword = tokenize_input(keyword, stop_words, important_keywords)
...
# Fungsi untuk menggantikan keyword dengan sinonimnya jika ada
def replace_keyword(word, synonyms):
    return synonyms.get(word, word)

# Fungsi untuk melakukan tokenisasi input
def tokenize_input(keyword, important_keywords, stop_words):
    try:
        # Konversi keyword ke lowercase
        lower_keyword = keyword.lower()
...
if __name__ == "__main__":
    # Pastikan argumen keyword disediakan
    if len(sys.argv) != 2:
        print("Usage: python nlp_processing.py <data>")
        sys.exit(1)

    # Baca keyword dan important keywords dari argumen pertama script
    data = json.loads(sys.argv[1])
    keyword = data['keyword']
    important_keywords = set(data['important_keywords'])
    ...
    # Jalankan fungsi tokenize_input
    processed_keyword = tokenize_input(keyword, important_keywords, stop_words)

    # Ganti kata-kata dengan sinonim jika perlu
    replaced_keyword = replace_keyword(processed_keyword, synonyms)
